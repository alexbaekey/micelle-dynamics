...
class lipid(object):\
    ...
    def update_pos(self,grid):
        ...
        if(self.disrupted==1):
            self.pos =(
                self.pos[0] + choice(np.arange(-30,30,1,dtype=int)),
                self.pos[1] + choice(np.arange(-30,30,1,dtype=int)))
            self.disrupted=0
        elif(self.Lstate==0 and self.Rstate==0 and self.Bstate==0):
            self.pos =(
            ...
            if(self.Lstate==0):
                if(
                    (self.Lpos[0]+randx < grid.max_x and self.Lpos[0]+randx > 0) and  
                    (self.Lpos[1]+randy < grid.max_y and self.Lpos[1]+randy > 0) and  
                    (abs(self.Lpos[0]+randx - self.pos[0])<=1 and abs(self.Lpos[1]+randy - self.pos[1])<=1)
                    ):
            ...
            if(self.Rstate==0):
                if(
                (self.Rpos[0]+randx < grid.max_x and self.Rpos[0]+randx > 0) and 
                (self.Rpos[1]+randy < grid.max_y and self.Rpos[1]+randy > 0) and  
                (abs(self.Rpos[0]+randx - self.pos[0])<=1 and abs(self.Rpos[1]+randy - self.pos[1])<=1)
                ):
            ...
            if(self.Bstate==0):
                if(
                (self.Bpos[0]+randx < grid.max_x and self.Bpos[0]+randx > 0) and 
                (self.Bpos[1]+randy < grid.max_y and self.Bpos[1]+randy > 0) and  
                (abs(self.Bpos[0]+randx - self.pos[0])<=1 and abs(self.Bpos[1]+randy - self.pos[1])<=1)
                ):
