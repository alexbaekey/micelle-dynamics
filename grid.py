...
class grid(object):
    ...
    def check_wall(self):
        ...
        if(self.Lassembled == 1 && self.Rassembled == 1):
            self.assembled = 1
            return current timestep
        else:
            return

    def print_grid(self,timestep,path,name):
    ...
    def radiate(radius,agents):
        ...
        for i in range(len(agents)):
            if( agents[i].pos[0]>radiation_x[0] && agents[i].pos[0]<radiation_x[1] && \
                agents[i].pos[1]>radiation_y[0] && agents[i].pos[1]<radiation_y[1]):
